...
turtles-own [
    ;; initialization of all turtle variables


    ;; the following variables are used for the alignment
    ;; of the turtles
...
;; Task 1: measure the global alignment coefficient
;; 1/N(sum of(the headings of the turtles deleted on the speed of the turtle))
to measure-global-alignment
  let sm 0
  let n 0
  ask turtles [
    set sm sm + heading
  ...
  set global-alignment sm / n
end
to-report global-alignment-v1
  report (sum [heading] of turtles) / count turtles
end
to-report global-alignment-v2
  report mean [heading] of turtles
end

;; Given a particle i, let Ai,n be the area of the Voronoi polygon for particle i at iteration n. We define the global clustering coefficient as:
;; (count{Ai < pi*R^2}) / N
;; where pi is the probability that a random point in the plane is inside the polygon Ai.
;; Task 2: measure the global clustering coefficient
to-report global-clustering-coefficient
  report (count [1] of turtles with [area < pi * 10 * 10]) / count turtles
end

;; measure the area of the Voronoi polygon for particle i at iteration n
to-report area
  report (count patches with [pcolor = myself]) * 1 * 1
end

;; Let’s pick a metric for alignment: polarization is the average angular deviation from the average direction of the flock. A flock is fully aligned when the polarization is 0 degrees and least aligned at 90 degrees.
;; Task 3: measure the global polarization
to-report global-polarization
  report mean [abs heading - global-alignment] of turtles
end

;; Implement another clustering metric called the Number of flocks. Let us define the new variable min-flock-distance. Use the idea of percolation clusters we used in the lecture to define flocks for each bird. The number of flocks and mean size are important measures.
;; Task 4: measure the number of flocks and the mean flock size
to-report number-of-flocks
  report count patches with [count turtles > 0]
end
to-report mean-flock-size
  report mean [count turtles] of patches with [count turtles > 0]
end
...
;; Take the Vicsek model as a start. Add predators and show how time-to-kill depends on the velocity and number of predators. The logic of predators - seek the closest bird and attack it. The velocity of predators is higher than of birds. When a predator is in the
;; range of sight, birds should fly to avoid the predator (think about realistic logic - the goal is to propose your own set of rules).

;; Task 1: measure the time to kill
;; Task 2: measure the number of birds killed
;; Task 3: measure the number of birds killed per predator

;; implement the following rules:
;; 1. If a bird is in the range of sight of a predator, it should fly away from the predators

;; Task 1 implementation
to-report time-to-kill
  report ticks
end

;; Task 2 implementation
to-report number-of-birds-killed
...
to-report number-of-birds-killed-per-predator
  report number-of-birds-killed / count predators
end


;; implementation the logic of predators: seek the closest bird and attack it
to predator
  ;find-closest-bird
  ifelse distance closest-bird < vision
    [ attack ][]
end

;; attack the closest bird
to attack
  turn-towards ([heading] of closest-bird) max-align-turn
  fd 1
end


;to find-closest-bird
 ; set closest-bird min-one-of turtles with [color = yellow] [distance myself]
;end

to-report closest-bird
  report min-one-of turtles with [color = yellow] [distance myself]
end

;; implementation the logic of birds: if a bird is in the range of sight of a predator, it should fly away from the predators
to bird
  ;find-closest-predator

  ifelse distance closest-predator < vision
    [ fly-away ] []
end

to-report closest-predator
  report min-one-of turtles with [color = red] [distance myself]
end

;; fly away from the closest predator
to fly-away
  turn-away ([heading] of closest-predator) max-align-turn
  fd 1
   ;; kill the closest bird but not a predator
    ifelse distance closest-bird < 0.1  and color != red
        [ die ][]
end

;; find the closest predator
;to find-closest-predator
 ; set closest-predator min-one-of predators [distance myself]
;end
...
to go
;; while birds are alive

    if count turtles with [color = yellow] = 0
        [ stop ]
  measure-global-alignment
    ask turtles [ flock ]
    ask turtles with [color = red] [ predator ]
    ask turtles with [color = yellow] [ bird ]
    ;; the following line is used to make the turtles
    ;; animate more smoothly.
    repeat 5 [ ask turtles [ fd 0.2 ] display ]
    ;; for greater efficiency, at the expense of smooth
...
"" ""
PENS
"default" 1.0 2 -16777216 true "" "plot global-alignment"
